...
// Initialize JavaScript for settings and setup interactions.
$PAGE->requires->js_call_amd('tool_redirectplus/settings', 'init', [[
    'apacheText' => get_string('apache_htaccess', 'tool_redirectplus'),
    'nginxText' => get_string('nginx', 'tool_redirectplus'),
    'pleskText' => get_string('plesk', 'tool_redirectplus'),
]]);
...
if ($tool_redirectplus_tab === 'settings') {
    ...
    echo html_writer::tag('label', get_string('behavior', 'tool_redirectplus'), ['for' => 'id_behavior', 'class' => 'font-weight-bold']);
    echo html_writer::select([
        'message' => get_string('behaviormessage', 'tool_redirectplus'),
        'redirect' => get_string('behaviorredirect', 'tool_redirectplus'),
    ], 'behavior', $tool_redirectplus_behavior, false, ['id' => 'id_behavior', 'class' => 'form-control']);
    echo html_writer::tag('div', get_string('behavior_desc', 'tool_redirectplus'), ['class' => 'form-text']);
    echo html_writer::end_div();

    // Custom message section (shown when behavior is 'message').
    echo html_writer::start_div('form-group mb-4', [
        'id' => 'message_section',
        'style' => 'display: ' . ($tool_redirectplus_behavior === 'message' ? 'block' : 'none') . ';',
    ]);
    ...
    // Redirect URL section (shown when behavior is 'redirect').
    echo html_writer::start_div('form-group mb-3', [
        'id' => 'redirect_section',
        'style' => 'display: ' . ($tool_redirectplus_behavior === 'redirect' ? 'block' : 'none') . ';',
    ]);
